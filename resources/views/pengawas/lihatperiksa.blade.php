...
<div class="container">
    <div class="d-flex justify-content-end mb-2">
        @if (Auth::check() && Auth::user()->role === 'pengawas')
            @if(in_array($pemeriksaan->kategori, ['SEHAT', 'CUKUP SEHAT']))
                <a href="{{ route('filesertifikat', $pemeriksaan->id_pemeriksaan) }}" class="btn btn-success me-2">
                    <i class="fas fa-certificate"></i> Generate Sertifikat
                </a>
            @endif
            <a href="{{ route('fileperiksa', $pemeriksaan->id_pemeriksaan) }}" class="btn btn-primary me-2">
                <i class="fas fa-file-alt"></i> Generate Berita Acara
            </a>
        @endif
        @if (!empty($pemeriksaan->file_ba))
            <a href="{{ asset('storage/' . $pemeriksaan->file_ba) }}" class="btn btn-secondary btn me-2" download>
        ...
        @if (!empty($pemeriksaan->file_sertifikat) && $userRole !== 'pengurus')
            <a href="{{ asset('storage/' . $pemeriksaan->file_sertifikat) }}" class="btn btn-secondary" download>
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header fw-bold">
                    Daftar Aspek Pemeriksaan
                </div>
                <div class="list-group list-group-flush">
                    <button class="list-group-item list-group-item-action fw-bold" onclick="showStep('step1')">Koperasi</button>
                    <button class="list-group-item list-group-item-action fw-bold" onclick="showStep('step2')">Tata Kelola</button>
                    <button class="list-group-item list-group-item-action fw-bold" onclick="showStep('step3')">Profil Resiko</button>
                    <button class="list-group-item list-group-item-action fw-bold" onclick="showStep('step4')">Kinerja Keuangan</button>
                    <button class="list-group-item list-group-item-action fw-bold" onclick="showStep('step5')">Permodalan</button>
                </div>
            </div>
        </div>

        <div class="col-md-8">
            <div id="step1" class="content-step">
                                        ...
                                        <div class="mt-3 border p-3 rounded bg-light">
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control" id="kas-bank-{{ $sectionIndex }}"
                                                    value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'kas_bank')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                                            </div>
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control" id="aktiva-{{ $sectionIndex }}"
                                                    value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'aktiva')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                                            </div>
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control" id="kewajiban-{{ $sectionIndex }}"
                                                    value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'kewajiban_lancar')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                                            </div>
                                        </div>
                                    @endif
                                @endforeach
                    ...
                    <div class="container mt-3">
                        <div class="row">
                            ...
                            <input type="text" class="form-control" id="ekuitas"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'ekuitas')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="pinjaman-usaha">2. Jumlah Pinjaman/Piutang Usaha</label>
                            <input type="text" class="form-control" id="pinjaman-usaha"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'pinjaman_usaha')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="kewajiban-ekuitas">3. Jumlah Kewajiban dan Ekuitas</label>
                            <input type="text" class="form-control" id="kewajiban-ekuitas"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'kewajiban_ekuitas')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="hutang-pajak">4. Hutang Pajak</label>
                            <input type="text" class="form-control" id="hutang-pajak"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'hutang_pajak')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="beban-masuk">5. Beban yang Masih Harus Dibayar</label>
                            <input type="text" class="form-control" id="beban-masuk"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'beban_masuk')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="hutang-biaya">6. Hutang Biaya/PKP-RI</label>
                            <input type="text" class="form-control" id="hutang-biaya"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'hutang_biaya')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="aktiva-lancar">7. Jumlah Aktiva Lancar</label>
                            <input type="text" class="form-control" id="aktiva-lancar"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'aktiva_lancar')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="persediaan">8. Persediaan Barang Dagang</label>
                            <input type="text" class="form-control" id="persediaan"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'persediaan')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="piutang-dagang">9. Piutang Dagang</label>
                            <input type="text" class="form-control" id="piutang-dagang"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'piutang_dagang')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="simpanan-pokok">10. Simpanan Pokok</label>
                            <input type="text" class="form-control" id="simpanan-pokok"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'simpanan_pokok')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="simpanan-wajib">11. Simpanan Wajib</label>
                            <input type="text" class="form-control" id="simpanan-wajib"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'simpanan_wajib')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="tabungan-anggota">12. Tabungan/simpanan anggota</label>
                            <input type="text" class="form-control" id="tabungan-anggota"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'tabungan_anggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            <input type="text" class="form-control" id="tabungan-nonanggota"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'tabungan_nonanggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            <input type="text" class="form-control" id="simpanan-jangkaanggota"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'simpanan_jangka_anggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            <input type="text" class="form-control" id="simpananjangka-calonanggota"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'simpanan_jangka_calonanggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="aktiva-lalu">16. Jumlah Aktiva Tahun Lalu</label>
                            <input type="text" class="form-control" id="aktiva-lalu"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'aktiva_lalu')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="ekuitas-lalu">17. Jumlah Ekuitas Tahun Lalu</label>
                            <input type="text" class="form-control" id="ekuitas-lalu"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'ekuitas_lalu')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="shu">18. Sisa Hasil Usaha Setelah Pajak</label>
                            <input type="text" class="form-control" id="shu"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'shu')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'partisipasi_bruto')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="beban-pokok">20. Jumlah Beban Pokok Anggota</label>
                            <input type="text" class="form-control" id="beban-pokok"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'beban_pokok')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="porsi-beban">21. Porsi Beban Usaha Anggota</label>
                            <input type="text" class="form-control" id="porsi-beban"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'porsi_beban')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'beban_perkoperasian')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="beban-usaha">23. Jumlah Beban Usaha</label>
                            <input type="text" class="form-control" id="beban-usaha"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'beban_usaha')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="shu-kotor">24. Sisa Hasil Usaha Kotor</label>
                            <input type="text" class="form-control" id="shu-kotor"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'shu_kotor')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="beban-penjualan">25. Beban Pokok Penjualan</label>
                            <input type="text" class="form-control" id="beban-penjualan"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'beban_penjualan')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="penjualan-anggota">26. Penjualan pada Anggota</label>
                            <input type="text" class="form-control" id="penjualan-anggota"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'penjualan_anggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'penjualan_nonanggota')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="pendapatan">28. Jumlah Pendapatan</label>
                            <input type="text" class="form-control" id="pendapatan"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'pendapatan')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                            <label for="shu-lalu">29. Sisa Hasil Usaha Setelah Pajak Tahun Lalu</label>
                            <input type="text" class="form-control" id="shu-lalu"
                            value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'shu_lalu')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                        </div>
                    </div>
            ...
            {{-- Step 5 --}}
            <div id="step5" class="content-step">
                    ...
                    <div class="container mt-3">
                        <div class="row">
                            ...
                            <input type="text" class="form-control" id="titipan-dana"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'titipan_dana')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                            ...
                            <input type="text" class="form-control rupiah" id="kewajiban-panjang" input type="text" class="form-control" id="titipan-dana"
                                value="{{ number_format($pemeriksaan->keuangan->where('aspek_keuangan', 'kewajiban_jangka_panjang')->first()->nominal ?? 0, 0, ',', '.') }}" readonly>
                            </div>
                        </div>
                    </div>
...
<script>
    function showStep(id) {
        ...
        document.querySelectorAll('.content-step').forEach(el => el.style.display = 'none');

        // Tampilkan yang diklik
        document.getElementById(id).style.display = 'block';
    }

    // Tampilkan default saat halaman dibuka
